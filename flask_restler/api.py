...
class Api(Blueprint):
    ...
    def register(self, app, options=None, first_registration=False):
        ...
        if self.specs:
            self.route("/_specs", params=dict(authorize=anonimous, update_specs=anonimous))(self.specs_view)
    ...
    def route(self, resource=None, url=None, url_detail=DEFAULT, params=None, **options):
        ...
        def wrapper(res):
            ...
            for _, (route_, endpoint_, options_) in res.meta.endpoints.values():
                api.add_url_rule(
                    "%s/%s" % (url_, route_.strip("/")),
                    view_func=view_func,
                    defaults={"endpoint": endpoint_},
                    **options_,
                )

            url_detail_ = url_detail
            if url_detail is DEFAULT:
                url_detail_ = res.meta.url_detail = res.meta.url_detail or ("%s/<%s>" % (url_, res.meta.name))
